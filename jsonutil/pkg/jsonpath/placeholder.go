...
//
// https://github.com/PaesslerAG/jsonpath/blob/c18d0f043db32b5d4295e14c6518fa9160e45d15/placeholder.go
//
// Licensed under a three-clause BSD-style license. A copy of the full license
// document is included in this distribution in the file `placeholder.go.LICENSE`.
...
func parseJSONObject(selectorMode selectorMode) func(ctx context.Context, p *gval.Parser) (gval.Evaluable, error) {
	return func(ctx context.Context, p *gval.Parser) (gval.Evaluable, error) {
		...
		for {
			...
			default:
				...
				e, err := parseJSONObjectElement(ctx, p, selectorMode, hasWildcard, key)
				if err != nil {
					return nil, err
				}
				evals.addElements(e)
...
func parseJSONObjectElement(ctx context.Context, gParser *gval.Parser, selectorMode selectorMode, hasWildcard bool, key gval.Evaluable) (jsonObject, error) {
	if hasWildcard {
		var p *parser
		switch gParser.Scan() {
		case '$':
