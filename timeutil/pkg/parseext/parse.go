...
// ParseInLocation uses all the formats in this tree to try to parse the given
// value in a particular time zone.
...
// Parse uses all the formats in this tree to parse the value in the local time
// zone.
...
// IsTime returns whether the given input would be considered a valid time
// value.
...
// ParseInLocation is a convenience function that calls
// Root().ParseInLocation().
...
func (tn *baseTimeNode) withZoneNode(length int) *baseTimeNode {
	for _, c := range tn.children {
	...
	base := &baseTimeNode{}
	c := &zoneTimeNode{baseTimeNode: base, len: length}
...
func (tn *baseTimeNode) withFracNode(length int) *baseTimeNode {
	return tn.withExpected(".").withDigitsNode(1, length, false)
}
